...
class AutonomousLoop:
    ...
    async def run(self):
        ...
        while True:
            try:
                ...
                await self.update_knowledge_graph(scenario, action, outcome)
    ...
    async def update_knowledge_graph(self, scenario, action, outcome):
        try:
            # Extract relevant information from scenario, action, and outcome
            entities = self.extract_entities(scenario, action, outcome)
            relationships = self.extract_relationships(scenario, action, outcome)
    ...
    def extract_entities(self, scenario, action, outcome):
        entities = []
        
        # Extract entities from scenario
        if isinstance(scenario, dict):
            for key, value in scenario.items():
                entities.append({'name': key, 'properties': {'value': str(value)}})
        
        # Extract entities from action
        if isinstance(action, dict):
            entities.append({'name': action.get('type', 'unknown_action'), 'properties': action})
        
        # Extract entities from outcome
        if isinstance(outcome, dict):
            entities.append({'name': 'outcome', 'properties': outcome})
        
        return entities

    def extract_relationships(self, scenario, action, outcome):
        relationships = []
        
        # Create a relationship between scenario and action
        relationships.append({
            'start': 'scenario',
            'end': action.get('type', 'unknown_action'),
            'type': 'LEADS_TO',
            'properties': {}
        })
        
        # Create a relationship between action and outcome
        relationships.append({
            'start': action.get('type', 'unknown_action'),
            'end': 'outcome',
            'type': 'RESULTS_IN',
            'properties': {}
        })
